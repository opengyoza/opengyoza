	...
	for _, tc := range cases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			for i := 0; i < len(myFlags); i++ {
				...
				case myFlags[i] == "-grpc-ca-file" && i+1 < len(myFlags):
					caPath = myFlags[i+1]
				case strings.HasPrefix(myFlags[i], "-grpc-ca-file="):
					caPath = strings.TrimPrefix(myFlags[i], "-grpc-ca-file=")
				}
				if caPath == "" {
					continue
				}
