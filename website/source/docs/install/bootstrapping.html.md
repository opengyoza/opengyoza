...
description: |-
  An agent can run in both client and server mode. Server nodes are responsible for running the consensus protocol and storing the cluster state. Before an OpenGyoza cluster can begin to service requests, a server node must be elected leader. Thus, the first nodes that are started are generally the server nodes. Bootstrapping is the process of joining these server nodes into a cluster.
---
...
Before an OpenGyoza cluster can begin to service requests, 
...
~> **Note**: In upstream Consul versions prior to 0.4, bootstrapping was a manual process. For details on using the `-bootstrap` flag directly, see the
...
newer versions of OpenGyoza (Consul 0.5 and newer) as it is more error-prone. 
...
configuration option. This option informs OpenGyoza of the expected number of
...
- Specify a list of servers with
  ...
  [start_join](/docs/agent/options.html#start_join)
  options.
- Specify a list of servers with [-retry-join](/docs/agent/options.html#_retry_join) option.
- Use automatic joining by tag for supported cloud environments with the [-retry-join](/docs/agent/options.html#_retry_join) option.
...
and run the `gyoza join` command.

```
$ gyoza join <Node A Address> <Node B Address> <Node C Address>
...
As a sanity check, the [`gyoza info`](/docs/commands/info.html) command
...
running [`gyoza info`](/docs/commands/info.html) on the followers, you
