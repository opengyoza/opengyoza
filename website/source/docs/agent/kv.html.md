---
layout: "docs"
page_title: "OpenGyoza KV"
sidebar_current: "docs-agent-kv"
description: |- 
    OpenGyoza KV is a core feature of OpenGyoza and is installed with the OpenGyoza agent.
---

# OpenGyoza KV 

OpenGyoza KV is a core feature of OpenGyoza and is installed with the OpenGyoza agent.
Once installed with the agent, it will have sane defaults. OpenGyoza KV allows
...
The OpenGyoza KV datastore is located on the servers, but can be accessed by any
...
requests to servers, including key/value reads and writes. Part of OpenGyoza’s
...
If you have not used OpenGyoza KV, check out this [Getting Started
...
The KV store can be accessed by the [gyoza kv CLI
subcommands](/docs/commands/kv.html), [HTTP API](/api/kv.html), and OpenGyoza UI.
...
The datastore itself is located on the OpenGyoza servers in the [data
directory](/docs/agent/options.html#_data_dir). To ensure data is not lost in
the event of a complete outage, use the [`gyoza
snapshot`](/docs/commands/snapshot/restore.html) feature to backup the data. 


## Using OpenGyoza KV

Objects are opaque to OpenGyoza, meaning there are no restrictions on the type of
...
## Extending OpenGyoza KV

### Consul Template

If you plan to use OpenGyoza KV as part of your configuration management process
review the [Consul
...
to be initiated on value changes to an OpenGyoza key. 

### Watches

OpenGyoza KV can also be extended with the use of watches.
...
OpenGyoza sessions can be used to build distributed locks with OpenGyoza KV. Sessions
...
Review the following guides to learn how to use OpenGyoza sessions for [application leader election](https://learn.hashicorp.com/consul/developer-configuration/elections) and 
to [build distributed semaphores](https://learn.hashicorp.com/consul/developer-configuration/semaphore).

### Vault

If you plan to use OpenGyoza KV as a backend for Vault, please review [this
