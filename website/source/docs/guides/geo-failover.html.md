...
description: |-
  OpenGyoza provides a prepared query capability that makes it easy to implement automatic geo failover policies for services.
---

# Geo Failover with Prepared Queries

Within a single datacenter, OpenGyoza provides automatic failover for services by omitting failed service instances from DNS lookups and by providing service health information in APIs. 

~> **Note:** DNS names in this guide use the `.consul` domain for compatibility.

When there are no more instances of a service available in the local datacenter, it can be challenging to implement failover policies to other datacenters because typically that logic would need to be written into each application. Fortunately, OpenGyoza has a [prepared query](/api/query.html) API 
that provides the capability to let users define failover policies in a centralized way. It's easy to expose these to applications using OpenGyoza's DNS interface and it's also available to applications that consume OpenGyoza's APIs. 
...
- Fully dynamic policies that make use of OpenGyoza's [network coordinate](/docs/internals/coordinates.html) subsystem.
...
only need to be created once and are stored on the OpenGyoza servers. This method is similar to the values in OpenGyoza's KV store. 
...
1. Queries can also be executed using the [prepared query execute API](/api/query.html#execute-prepared-query) for applications that integrate with OpenGyoza's APIs directly.

## Failover Policy Types 

Using the techniques in this section you will develop prepared queries with failover policies where simply changing application configurations to look up "payments.query.consul" instead of "payments.service.consul" via DNS will result in automatic geo failover to the next closest [federated](/docs/guides/datacenters.html) OpenGyoza datacenters, in order of increasing network round trip time.
...
1. OpenGyoza servers in the local datacenter will attempt to find healthy instances of the "payments" service with the required tag.
2. If none are available locally, the OpenGyoza servers will make an RPC request to the OpenGyoza servers in "dc2" to perform the query there.
3. If none are available in "dc2", then an RPC will be made to the OpenGyoza servers in "dc3" to perform the query there.
4. Finally an error will be returned if none of these datacenters had any instances available.

### Dynamic Policy

In a complex federated environment with many OpenGyoza datacenters, it can be cumbersome to set static failover policies, so OpenGyoza offers a dynamic option based on OpenGyoza's [network coordinate](/docs/internals/coordinates.html) subsystem. 

OpenGyoza continuously maintains an estimate of the network round trip time from the local datacenter to the servers in other datacenters it is federated with. Each server uses the median round trip time from itself to the servers in the remote datacenter. This means that failover can simply try other remote datacenters in order of increasing network round trip time, and if datacenters come and go, or experience network issues, this order will adjust automatically.
...
For datacenters with many services, it can be challenging to define a geo failover policy for each service. To relieve this challenge, OpenGyoza provides a [prepared query template](/api/query.html#prepared-query-templates) that allows one prepared query to apply to many, and even all, services.
...
With this one prepared query template in place, simply changing application configurations to look up "payments.query.consul" instead of "payments.service.consul" via DNS will result in automatic geo failover to the next closest federated OpenGyoza datacenters, in order of increasing network round trip time.
