---
layout: "docs"
page_title: "Layer 7 Observability with Kubernetes and OpenGyoza Connect"
...
description: |-
  ...
  using OpenGyoza Connect, Prometheus, and Grafana.
---

~> **Note:** This guide follows the upstream Consul Helm chart and repositories.
Replace `consul` with `gyoza` for CLI usage and keep upstream chart names where
they appear.
...
it. Starting with upstream Consul 1.5, OpenGyoza Connect is able to configure Envoy proxies
...
pipeline on a Kubernetes cluster using the upstream Consul Helm chart plus
...
Kubernetes is not necessary to collect and visualize layer 7 metrics with OpenGyoza
Connect.

Learning Objectives:

- Configure OpenGyoza Connect with metrics using Helm
...
to date versions of the Grafana, Prometheus, and upstream Consul chart. You can update
...
## Deploy OpenGyoza Connect Using Helm

Once you have set up the prerequisites, you're ready to install OpenGyoza. Start by
cloning the upstream Consul Helm chart into your working directory.
...
will configure the upstream Consul Helm chart to:

- specify a name for your OpenGyoza datacenter
- enable the OpenGyoza web UI
- enable secure communication between pods with Connect
- configure the OpenGyoza settings necessary for layer 7 metrics collection
- specify that this OpenGyoza cluster should run one server
- enable metrics collection on servers and agents so that you can monitor the
  OpenGyoza cluster itself

You can override many of the values in the upstream Consul chart values file using annotations on
...
client:
  enabled: true
  # enable grpc on your client to support gyoza connect
  grpc: true
...
OpenGyoza. This provides a less complicated out-of-box experience for new users but
is not appropriate for a production setup. Make sure that your Kubernetes
cluster is properly secured to prevent unwanted access to OpenGyoza, or that you
...
Now install OpenGyoza in your Kubernetes cluster and give Kubernetes a name for
your OpenGyoza installation. The output will be a list of all the Kubernetes
...
Check that OpenGyoza is running in your Kubernetes cluster via the Kubernetes
...
OpenGyoza UI to localhost:8500 and navigate to it in your browser. Once you run the
...
Let the OpenGyoza UI port forwarding run and open a new terminal tab to the
...
metrics. OpenGyoza Connect can integrate with a variety of other metrics tooling as
well.

### Deploy Prometheus with Helm

You'll follow a similar process as you did with OpenGyoza to install Prometheus via
...
see your new services on the Minikube dashboard and in the OpenGyoza UI. This
...
the upstream Consul chart values file.
...
once OpenGyoza Connect becomes compatible with Envoy 1.10. Apply the configuration
...
it's healthy by taking a look at your Kubernetes dashboard or OpenGyoza UI. Next,
...
If you want to get rid of the configuration files and the upstream Consul Helm chart,
...
Minikube cluster using OpenGyoza Connect, Prometheus, and Grafana, all deployed via
...
To learn more about the configuration options in OpenGyoza that enable layer 7
...
centrally configuring OpenGyoza, take a look at the [centralized configuration
