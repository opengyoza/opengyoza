...
description: |-
  OpenGyoza can be installed to the Kubernetes minikube tool for local development.
---

# OpenGyoza Installation to Minikube via Helm
...
In this guide, you'll start a local Kubernetes cluster with minikube. You'll install OpenGyoza with only a few commands, then deploy two custom services that use OpenGyoza to discover each other over encrypted TLS via OpenGyoza Connect. Finally, you'll tighten down OpenGyoza Connect so that only the approved applications can communicate with each other.

[Demo code](https://github.com/hashicorp/demo-consul-101) is available.

- [Task 1: Start Minikube and Install OpenGyoza with Helm](#task-1-start-minikube-and-install-opengyoza-with-helm)
- [Task 2: Deploy an OpenGyoza-aware Application to the Cluster](#task-2-deploy-an-opengyoza-aware-application-to-the-cluster)
- [Task 3: Configure OpenGyoza Connect](#task-3-use-opengyoza-connect)

## Prerequisites

Let's install OpenGyoza on Kubernetes with minikube. This is a relatively quick and easy way to try out OpenGyoza on your local machine without the need for any cloud credentials. You'll be able to use most OpenGyoza features right away.
...
## Task 1: Start Minikube and Install OpenGyoza with Helm
...
### Step 2: Install the OpenGyoza Helm Chart to the Cluster
...
Now we're ready to install OpenGyoza to the cluster, using the `helm` tool. Initialize Helm with `helm init`. You'll see a note that Tiller (the server-side component) has been installed. You can ignore the policy warning.
...
Now we need to install OpenGyoza with Helm. To get the freshest copy of the upstream Helm chart, clone the [hashicorp/consul-helm](https://github.com/hashicorp/consul-helm) repository.
...
We've given a name to the datacenter running this OpenGyoza cluster. We've enabled the OpenGyoza web UI via a `NodePort`. When deploying to a hosted cloud that implements load balancers, we could use `LoadBalancer` instead. We'll enable secure communication between pods with Connect. We also need to enable `grpc` on the client for Connect to work properly. Finally, specify that this OpenGyoza cluster should only run one server (suitable for local development).
...
# Enable the OpenGyoza Web UI via a NodePort
ui:
  service:
    type: "NodePort"
...
# Use only one OpenGyoza server for local development
server:
  replicas: 1
  bootstrapExpect: 1
  disruptionBudget:
...
## Task 2: Deploy an OpenGyoza-aware Application to the Cluster

### Step 1: View the OpenGyoza Web UI

Verify the installation by going back to the Kubernetes dashboard in your web browser. Find the list of services. Several include `gyoza` in the name and have the `app: gyoza` label.

![Minikube Dashboard with OpenGyoza](/assets/images/guides/minikube-dashboard-consul.png "Minikube Dashboard with OpenGyoza")

There are a few differences between running Kubernetes on a hosted cloud vs locally with minikube. You may find that any load balancer resources don't work as expected on a local cluster. But we can still view the OpenGyoza UI and other deployed resources.
...
You can now view the OpenGyoza web UI with a list of OpenGyoza's services, nodes, and other resources.

![Minikube OpenGyoza UI](/assets/images/guides/minikube-consul-ui.png "Minikube OpenGyoza UI")
...
Visit http://localhost:9002 in your web browser. You'll see a running `dashboard` container in the kubernetes cluster that displays a number retrieved from the `counting` service using OpenGyoza service discovery and secured over the network by TLS via an Envoy proxy.

![Application Dashboard](/assets/images/guides/minikube-app-dashboard.png "Application Dashboard")

### Addendum: Review the Code

Let's take a peek at the code. Relevant to this Kubernetes deployment are two YAML files in the `04` directory. The `counting` service defines an `annotation` in the `metadata` section that instructs OpenGyoza to spin up an OpenGyoza Connect proxy for this service: `connect-inject`. The relevant port number is found in the `containerPort` section (`9001`). This Pod registers an OpenGyoza service that will be available via a secure proxy.
...
And there we have OpenGyoza running on a Kubernetes cluster, as demonstrated by two services which communicate with each other via OpenGyoza Connect and an Envoy proxy.

![Success](/assets/images/guides/minikube-connect-success.png "Success")

## Reference

For more on OpenGyoza's integration with Kubernetes (including multi-cloud, service sync, and other features), see the [OpenGyoza with Kubernetes](/docs/platform/k8s/index.html) documentation.
