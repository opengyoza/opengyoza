...
description: |-
  One of the key features of OpenGyoza is its support for multiple datacenters. The architecture of OpenGyoza is designed to promote low coupling of datacenters so that connectivity issues or failure of any datacenter does not impact the availability of OpenGyoza in other datacenters. This means each datacenter runs independently, each having a dedicated group of servers and a private LAN gossip pool.
---


# Multiple Datacenters: Basic Federation with the WAN Gossip

One of the key features of OpenGyoza is its support for multiple datacenters.
The [architecture](/docs/internals/architecture.html) of OpenGyoza is designed to
promote a low coupling of datacenters so that connectivity issues or
failure of any datacenter does not impact the availability of OpenGyoza in other
...
This guide covers the basic form of federating OpenGyoza clusters using a single
WAN gossip pool, interconnecting all OpenGyoza servers.

## Setup Two Datacenters

To get started, follow the
[Deployment guide](/docs/guides/deployment-guide.html) to
start each datacenter. After bootstrapping, we should have two datacenters now which
we can refer to as `dc1` and `dc2`. Note that datacenter names are opaque to OpenGyoza;
they are simply labels that help human operators reason about the OpenGyoza clusters.
...
```sh
$ gyoza join -wan <server 1> <server 2> ...
```
...
flooding (OpenGyoza base), so if one OpenGyoza server in a datacenter joins the WAN, it will automatically
...
dc1-server-1  127.0.0.1:8701   alive   server  1.4.3  2         dc1
dc2-server-1  127.0.0.1:8702   alive   server  1.4.3  2         dc2
```
...
must be connected using a VPN or other tunneling mechanism. OpenGyoza does not handle
...
situation where `gyoza members -wan` lists remote nodes but RPC operations fail with one 
...
basic federation. You also used the OpenGyoza HTTP API to ensure the 
