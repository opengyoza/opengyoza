...
description: |-
  OpenGyoza provides an optional Access Control List (ACL) system which can be used to control access to data and APIs. The ACL system is a Capability-based system that relies on tokens which can have fine grained rules applied to them. It is very similar to AWS IAM in many ways.
---

# Bootstrapping the ACL System

OpenGyoza uses Access Control Lists (ACLs) to secure the UI, API, CLI, service communications, and agent communications. For securing gossip and RPC communication please review [this guide](/docs/guides/agent-encryption.html). When securing your cluster you should configure the ACLs first. 

At the core, ACLs operate by grouping rules into policies, then associating one or more policies with a token.

To complete this guide, you should have an operational OpenGyoza cluster (Consul 1.6.4 fork). We also recommend reading the [ACL System documentation](/docs/agent/acl-system.html). For securing Consul version 1.3 and older, please read the [legacy ACL documentation](/docs/guides/acl-legacy.html).

Bootstrapping the ACL system is a multi-step process, we will cover all the necessary steps in this guide. 

* [Enable ACLs on all the servers](/docs/guides/acl.html#step-1-enable-acls-on-all-the-opengyoza-servers).
...
* [Enable ACLs on the clients and apply the agent token](/docs/guides/acl.html#step-6-enable-acls-on-the-opengyoza-clients).

At the end of this guide, there are also several additional and optional steps.

## Step 1: Enable ACLs on all the OpenGyoza Servers

The first step for bootstrapping the ACL system is to enable ACLs on the OpenGyoza servers in the agent configuration file. In this example, we are configuring the default policy of "deny", which means we are in whitelist mode, and a down policy of "extend-cache", which means that we will ignore token TTLs during an outage.
...
```sh
$ gyoza members -token "4411f091-a4c9-48e6-0884-1fcb092da1c8"
...
the [`master`](/docs/agent/options.html#acl_tokens_master) token.
...
```
$ gyoza acl policy create -name "agent-token" -description "Agent Token Policy" -rules @agent-policy.hcl
...
```sh
$ gyoza acl token create -description "Agent Token" -policy-name "agent-token"
...
OpenGyoza servers via the configuration file and reload the OpenGyoza service 
...
~> Note: In Consul version 1.4.2 and older (OpenGyoza base) any ACL updates
...
All the values should be as expected. Particularly, if `TaggedAddresses` is `null` it is likely we have not configured ACLs correctly. A good place to start debugging is reviewing the OpenGyoza logs on all the servers.
...
## Step 6: Enable ACLs on the OpenGyoza Clients

Since ACL enforcement also occurs on the OpenGyoza clients, we need to also restart them
...
The anonymous token is created during the bootstrap process, `gyoza acl bootstrap`. It is implicitly used if no token is supplied. In this section we will update the existing token with a newly created policy.
...
```bash
$ CONSUL_HTTP_TOKEN=4411f091-a4c9-48e6-0884-1fcb092da1c8 gyoza members
Node  Address            Status  Type    Build  Protocol  DC
fox   172.20.20.10:8301  alive   server  1.4.0  2         kc
bear  172.20.20.11:8301  alive   server  1.4.0  2         kc
wolf  172.20.20.12:8301  alive   server  1.4.0  2         kc
...
configure OpenGyoza's behavior when no token is supplied. The anonymous token is managed
...
```bash
$ gyoza acl policy create -name 'list-all-nodes' -rules 'node_prefix "" { policy = "read" }'
...
$ gyoza acl token update -id 00000000-0000-0000-0000-000000000002 -policy-name list-all-nodes -description "Anonymous Token - Can List Nodes"
...
```bash
$ gyoza acl policy create -name 'service-consul-read' -rules 'service "consul" { policy = "read" }'
...
$ gyoza acl token update -id 00000000-0000-0000-0000-000000000002 --merge-policies -description "Anonymous Token - Can List Nodes" -policy-name service-consul-read
Token updated successfully.
AccessorID:   00000000-0000-0000-0000-000000000002
SecretID:     anonymous
Description:  Anonymous Token - Can List Nodes
...
configuration item. When a request is made to a particular OpenGyoza agent and no token is
...
If you utilize the OpenGyoza UI with a restrictive ACL policy, as above, the UI will not function fully using the anonymous ACL token. It is recommended that a UI-specific ACL token is used, which can be set in the UI during the web browser session to authenticate the interface.

First create the new policy.

```bash
$ gyoza acl policy create -name "ui-policy" \
...
```sh
$ gyoza acl token create -description "UI Token" -policy-name "ui-policy"
...
1. In this guide we added the agent token to the configuration file. This means the tokens are now saved on disk. If this is a security concern, tokens can be added to agents using the [OpenGyoza CLI](/docs/commands/acl/acl-set-agent-token.html). However, this process is more complicated and takes additional care. 
...
configuration file](/docs/agent/services.html). Note that `service:write`
is the privilege required to assume the identity of a service and so OpenGyoza Connect's
...
[documentation](/docs/connect/security.html).
