...
description: |-
  Consul Template provides a programmatic method for rendering configuration files from OpenGyoza (Consul-compatible) data.
---

# Consul Template

~> Consul Template is an upstream tool with its own releases. OpenGyoza remains
   API-compatible with Consul KV, so the same workflows apply.

The Consul template tool provides a programmatic method
for rendering configuration files from a variety of locations,
including OpenGyoza KV. It is an ideal option for replacing complicated API
...
infrastructure on machines many not be able to directly connect to the OpenGyoza cluster.

1. *Discover data about the OpenGyoza cluster and service*. It is possible to collect
information about the services in your OpenGyoza cluster. For example, you could
...
[OpenGyoza KV](/docs/commands/kv.html)
...
reads one or more template files and queries OpenGyoza for all
...
reduce the load on OpenGyoza servers to share where possible.

## Install Consul Template

For this guide, we are using a local OpenGyoza agent in development
mode which can be started with `gyoza agent -dev`. To quickly set
up a local OpenGyoza agent, refer to the [install guide](/docs/install/index.html). The
OpenGyoza agent must be running to complete all of the following
steps.

The Consul template tool is not included with the OpenGyoza binary and will
...
## Use Case: OpenGyoza KV

In this first use case example, we will render a template that pulls the HashiCorp address
from OpenGyoza KV. To do this we will create a simple template that contains the HashiCorp
address, run `consul-template`, add a value to OpenGyoza KV for HashiCorp's address, and
...
Finally, open a new terminal so we can write data to the key in OpenGyoza using the command
line interface.

```shell
$ gyoza kv put hashicorp/street_address "101 2nd St"
...
immediately. Go ahead and try `gyoza kv put hashicorp/street_address "22b Baker ST"`.
...
In this use case example, we will discover all the services running in the OpenGyoza cluster.
...
still see the `gyoza` service when viewing `all-services.txt`.
