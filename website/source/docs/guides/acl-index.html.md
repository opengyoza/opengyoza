...
description: |-
  OpenGyoza provides an optional Access Control List (ACL) system which can be used to control access to data and APIs. Select the following guide for your use case.
---

# ACL Documentation and Guides

OpenGyoza uses Access Control Lists (ACLs) to secure the UI, API, CLI, service communications, and agent communications. At the core, ACLs operate by grouping rules into policies, then associating one or more policies with a token.

~> **Note:** This guide retains upstream Consul version labels and `CONSUL_*` environment
...
OpenGyoza provides an optional Access Control List (ACL) system which can be used to control access to data and APIs. The ACL system is a Capability-based system that relies on tokens which can have fine grained rules applied to them. The [ACL System documentation] details the functionality of OpenGyoza ACLs.
...
We have several guides for setting up and configuring OpenGyoza's ACL system. They include how to bootstrap the ACL system in OpenGyoza (Consul 1.4.0+ base). Please select one of the following guides to get started.
...
Learn how to control access to OpenGyoza resources with this step-by-step [guide](https://learn.hashicorp.com/consul/advanced/day-1-operations/acl-guide) on bootstrapping the ACL system in Consul 1.4.0 and newer (applies to OpenGyoza). This guide also includes additional steps for configuring the anonymous token, setting up agent-specific default tokens, and creating tokens for the UI.

### Securing OpenGyoza with ACLs
