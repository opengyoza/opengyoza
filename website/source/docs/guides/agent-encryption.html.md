...
To complete the RPC encryption section, you must have [configured agent certificates](/docs/guides/creating-certificates.html).

## Gossip Encryption

To enable gossip encryption, you need to use an encryption key when starting the OpenGyoza agent. The key can be simple set with the `encrypt` parameter in the agent configuration file. Alternatively, the encryption key can be placed in a seperate configuration file with only the `encrypt` field, since the agent can merge multiple configuration files. The key must be 32-bytes, Base64 encoded. 

You can use the OpenGyoza CLI command, [`gyoza keygen`](/docs/commands/keygen.html), to generate a cryptographically suitable key.
...
agent configuration file and then pass the file at startup with the [`-config-dir`](/docs/agent/options.html#_config_dir) flag.
...
```sh
$ gyoza agent -config-dir=/etc/consul.d/
==> Starting OpenGyoza agent...
==> Starting OpenGyoza agent RPC...
...
**Step 1**: Generate an encryption key using `gyoza keygen`.
...
A rolling update can be made by restarting the OpenGyoza agents (clients and servers) in turn. `gyoza reload` or `kill -HUP <process_id>` is _not_ sufficient to change the gossip configuration.

**Step 3**: Update the `encrypt_verify_outgoing` setting to `true` and perform another rolling update of the cluster by restarting OpenGyoza on each agent. The agents will now be sending encrypted gossip but will still allow incoming unencrypted traffic.
...
OpenGyoza supports using TLS to verify the authenticity of servers and clients. To enable TLS,
OpenGyoza requires that all servers have certificates that are signed by a single
...
Review the [docs for specifics](/docs/agent/encryption.html).

In Consul version 0.8.4 and newer (OpenGyoza base), migrating to TLS encrypted traffic on a running cluster
is supported. 
...
The `verify_outgoing` parameter enables agents to verify the authenticity of OpenGyoza servers for outgoing connections. The `verify_server_hostname` parameter requires outgoing connections to perform hostname verification and is critically important to prevent compromised client agents from becoming servers and revealing all state to the attacker. Finally, the `verify_incoming` parameter enables the servers to verify the authenticity of all incoming connections. 
...
Next, perform a rolling restart of each agent in the cluster. After this step, TLS should be enabled everywhere but the agents will not yet be enforcing TLS. Again, `gyoza reload` or `kill -HUP <process_id>` is _not_ sufficient to update the configuration.


**Step 2**: Change the `verify_incoming`, `verify_outgoing`, and `verify_server_hostname` to `true` then perform another rolling restart of each agent in the cluster.
...
the [agent configuration](/docs/agent/options.html#ports).
