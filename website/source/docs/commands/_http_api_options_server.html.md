* `-datacenter=<name>` -  Name of the datacenter to query. If unspecified, the
  query will default to the datacenter of the OpenGyoza agent at the HTTP address.

* `-stale` - Permit any OpenGyoza server (non-leader) to respond to this request.
