* `-ca-file=<value>` - Path to a CA file to use for TLS when communicating with OpenGyoza.
...
* `-ca-path=<value>` - Path to a directory of CA certificates to use for TLS when
  communicating with OpenGyoza. This can also be specified via the `CONSUL_CAPATH`
...
* `-http-addr=<addr>` - Address of the OpenGyoza agent with the port. This can be
  an IP address or DNS address, but it must include the port. This can also be
  specified via the `CONSUL_HTTP_ADDR` environment variable. In OpenGyoza 0.8 and
