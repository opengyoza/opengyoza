...
## Q: What is Checkpoint? / Does OpenGyoza call home?

OpenGyoza makes use of a HashiCorp service called [Checkpoint](http://checkpoint.hashicorp.com)
...
## Q: Does OpenGyoza rely on UDP Broadcast or Multicast?

OpenGyoza uses the [Serf](https://www.serf.io) gossip protocol which relies on
TCP and UDP unicast. Broadcast and Multicast are rarely available in a multi-tenant
or cloud network environment. For that reason, OpenGyoza and Serf were both
designed to avoid any dependence on those capabilities.

## Q: Is OpenGyoza eventually or strongly consistent?

OpenGyoza has two important subsystems, the service catalog and the gossip protocol.
...
## Q: Does OpenGyoza support delta updates for watchers or blocking queries?

OpenGyoza does not currently support sending a delta or a change only response
...
By design, OpenGyoza offloads this to clients instead of attempting to support
...
## Q: What network ports does OpenGyoza use?

The [Ports Used](/docs/agent/options.html#ports) section of the Configuration documentation lists all ports that OpenGyoza uses.

## Q: Does OpenGyoza require certain user process resource limits?

There should be only a small number of open file descriptors required for a
OpenGyoza client agent. The gossip layers perform transient connections with
...
of the OpenGyoza servers. A small number of file descriptors are also required
for watch handlers, health checks, log files, and so on.

For an OpenGyoza server agent, you should plan on the above requirements and
...
The default ulimits are usually sufficient for OpenGyoza, but you should closely
...
## Q: What is the per-key value size limitation for OpenGyoza's key/value store?
...
than that limit in a value. It should be noted that the OpenGyoza key/value store
...
## Q: What data is replicated between OpenGyoza datacenters?
...
situations where a limited subset of data can be replicated, such as with upstream Consul's built-in
